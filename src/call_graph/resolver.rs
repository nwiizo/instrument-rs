...
impl SymbolResolver {
    ...
    pub fn register_import(&mut self, local_name: &str, full_path: &str) {
        self.imports.insert(local_name.to_string(), full_path.to_string());
    }
    ...
    fn is_external_path(&self, path: &Path) -> bool {
        if let Some(first_segment) = path.segments.first() {
            ...
            matches!(
                ...
                "tokio" | "async_trait" | "serde" | "anyhow" |
                "thiserror" | "log" | "tracing" | "futures"
    ...
    fn collect_use_path(&mut self, tree: &syn::UseTree, path: &mut Vec<String>) {
        match tree {
            ...
            syn::UseTree::Name(use_name) => {
                ...
                self.imports.insert(local_name, format!("{}::{}", full_path, use_name.ident));
            }
            syn::UseTree::Rename(use_rename) => {
                let renamed = use_rename.rename.to_string();
                let full_path = path.join("::");
                self.imports.insert(renamed, format!("{}::{}", full_path, use_rename.ident));
            }
            _ => {}
        }
    }
