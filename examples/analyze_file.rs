...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for func in &result.functions {
        ...
        println!("  - Return type: {}", func.return_type.as_ref().unwrap_or(&"()".to_string()));
        println!("  - Complexity:");
        ...
        println!("    - Result returns: {}", func.error_handling.result_returns);
        println!("    - unwrap() calls: {}", func.error_handling.unwrap_calls);
        println!("    - ? operators: {}", func.error_handling.question_mark_ops);
        
        if !func.calls.is_empty() {
            println!("  - Calls:");
            for call in &func.calls {
                println!("    - {} ({})", call.callee, if call.is_method { "method" } else { "function" });
            }
        }
    }
