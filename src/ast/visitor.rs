...
use syn::{
    visit::{self, Visit},
    spanned::Spanned,
    Attribute, Block, Expr, File, Item, ItemFn,
    ReturnType, Stmt, Type, ImplItem, BinOp,
};
use proc_macro2::Span;
use quote::{quote, ToTokens};

use crate::ast::{
    AnalysisResult, CallInfo, ComplexityMetrics, ElementKind, ErrorHandlingInfo,
    FunctionInfo, InstrumentableElement, Location, ModuleInfo, SourceFile,
};
...
impl AstVisitor {
    ...
    pub fn new(source_file: SourceFile) -> Self {
        let source_lines: Vec<String> = source_file.source.lines()
            .map(String::from)
            .collect();
    ...
    fn is_result_or_option_type(&self, ty: &ReturnType) -> (bool, bool) {
        match ty {
            ...
            ReturnType::Type(_, ref ty) => {
                ...
                let is_result = type_str.contains("Result") || type_str.contains("std::result::Result");
                let is_option = type_str.contains("Option") || type_str.contains("std::option::Option");
                (is_result, is_option)
            }
        }
    }
    ...
    fn add_element(&mut self, kind: ElementKind, span: Span) {
        let element = InstrumentableElement {
            ...
            parent_id: self.context.current_function.as_ref().map(|f| f.info.id.clone()),
